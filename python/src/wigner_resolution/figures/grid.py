...
All heatmap columns use ``aspect='equal'`` so overlay ellipses keep their
    ...
    *,
    ...
    for i, state in enumerate(states):
        ...
        wigner_heatmap(ax_h, state,
                       show_capacity=show_capacity, show_quantum_blob=show_quantum_blob)
...
def assemble_grid_5col(
    ...
    *,
    ...
    Layout per row, left to right:
      ...
      Col 2: W(x, 0).
      Col 3: K_{π/2}(x, p) with capacity A and quantum blob
             β_{π/2} overlays.
      Col 4: P_{π/2}(x, 0).
      Col 5: W̃(x, p) with capacity A and resolution a overlays.

    Cols 1, 3, 5 are aspect-equal phase-space heatmaps anchored at
    state.overlay_center_x. Cols 2 and 4 are cross-sections.
    ...
    for i, state in enumerate(states):
        ...
        wigner_heatmap(
            ax_h, state,
            show_capacity=True, show_quantum_blob=False,
        ...
        # Col 3: capacity + quantum blob β_{π/2}.
        quantum_kernel_heatmap(
        ...
        # Col 5: capacity + resolution.
        tilde_W_heatmap(
            ax_t, state,
            n_theta=tilde_W_n_theta,
            show_capacity=show_capacity, show_quantum_blob=False,
            show_resolution=True,
        )
