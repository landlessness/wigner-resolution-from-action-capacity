...
\caption{\textbf{Heisenberg cell, quantum blobs at principal axes, and quorum cell.} The Heisenberg cell is the state's covariance ellipsoid with action capacity $A = \pi \, \Delta x \, \Delta p \ge h/2$. The quantum blob family (Fig.~\ref{fig:quantum_blobs}) has two members at the principal axes, $\beta_{\theta=0} = \pi \, \Delta x \, \delta p = h/2$ and $\beta_{\pi/2} = \pi \,\delta x \,\Delta p = h/2$. Zurek's relations $\delta p \sim \hbar/\Delta x$ and $\delta x \sim \hbar/\Delta p$ locate the semi-axis scales from the state's action capacity. The quorum cell $a$ is an ellipse with semi-axes $\delta x, \delta p$ and action $a = \pi \, \delta x \, \delta p = \pi\hbar^2/(\Delta x\,\Delta p)$. $A$ circumscribes $\beta_{\theta=0}$ and $\beta_{\pi/2}$, which circumscribe $a$.}
...
Between these principal members the family $\{\beta_\theta\}$ deforms across $\theta \in [0,\pi)$ at constant action $h/2$ (Fig.~\ref{fig:quantum_blobs}). The Heisenberg cell $A$ circumscribes each $\beta_\theta$. In the scaled coordinates $X = x/\Delta x$, $P = p/\Delta p$ it is one ellipse turned through $\theta$,
\begin{equation}
...
\label{eq:wigner}
\end{equation}
The Wigner function of each quantum blob $\beta_\theta$ is the centered Gaussian whose $h/2$ ellipse is $\beta_\theta$,
\begin{equation}
...
\label{eq:Wtheta}
\end{equation}
Convolving $W$ with the kernel $K_\theta$ of a single quantum blob $\beta_\theta$ lifts it to Planck's quantum of action and yields a non-negative function~\cite{cartwright1976,degosson2005bullsci,degossonluef2009irruption,cordero2018}.

%===============================================================================
%  ALL-ANGLES RESOLUTION: QUORUM CELL
%===============================================================================
Zurek also located the scale $a \sim \hbar \times (\hbar / A)$~\cite{zurek2001,zurek2003decoherence} of the chessboard structure in the compass state. We identify its rotationally invariant form as the quorum cell $a$ (Fig.~\ref{fig:cat}, column 1). Named after the quorum of observables in optical tomography~\cite{darianomacconeparis2001}, $a$ is the ellipse with semi-axes $\delta x, \delta p$ (Figs.~\ref{fig:principal_blobs},~\ref{fig:quantum_blobs}) and action
\begin{equation}
...
column 1) and the symplectic polar dual of the Heisenberg cell $A$~\cite{degosson2022polar,degosson2024reconstruction}. The duality is reflexive, so $A$ and $a$ each fix the other, and antimonotone, so $a$ shrinks as $A$ grows. Each blob $\beta_\theta$ is circumscribed by $A$ and circumscribes $a$, nesting as $A \supseteq \beta_\theta \supseteq a$ (Fig.~\ref{fig:quantum_blobs}). Resolution and capacity are reciprocal at the quantum of action, $a\,A = (h/2)^2$.
...
quorum cell $a$, so no $h/2$ smoothing of this state resolves
...
is $a$, reached by the family $\{\beta_\theta\}$ across
$\theta \in [0,\pi)$.
