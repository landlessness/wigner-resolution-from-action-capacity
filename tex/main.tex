...
Between these principal members the family $\{a_\theta\}$ deforms across $\theta \in [0,\pi)$ at constant action $h/2$ (Fig.~\ref{fig:quantum_blobs}). The Heisenberg cell $A$ circumscribes each $a_\theta$. In the scaled coordinates $X = x/\Delta x$, $P = p/\Delta p$ it is one ellipse turned through $\theta$,
\begin{equation}
...
column 1) and the symplectic polar dual of the Heisenberg cell $A$~\cite{degosson2022polar,degosson2024reconstruction}. The duality is reflexive, so $A$ and $\tilde a$ each fix the other, and antimonotone, so $\tilde a$ shrinks as $A$ grows. Each blob $a_\theta$ is circumscribed by $A$ and circumscribes $\tilde a$, nesting as $A \supseteq a_\theta \supseteq \tilde a$ (Fig.~\ref{fig:quantum_blobs}). Resolution and capacity are reciprocal at the quantum of action, $\tilde a\,A = (h/2)^2$.

% ===============================================================================
%  THE CONVOLVED PORTRAIT
% ===============================================================================
A single blob commits to a quadrature the state does not favor. Integrating $\widetilde W_\theta$ over the family favors none, yielding the phase-space portrait,
\begin{equation}
