...
\caption{\textbf{Capacity, quadrature family of quantum blobs, and resolution.} Action capacity $A$ circumscribes the quantum blob family $\{\beta_\theta\}$, which circumscribes resolution $a$. Across $\theta$ the blob family deforms at constant action $\beta_\theta = h/2$. At the principal angles $\theta = 0$ and $\theta = \pi/2$, the family recovers $\beta_{\theta=0}$ and $\beta_{\pi/2}$ from Fig.~\ref{fig:principal_blobs}.}
...
% ===============================================================================
Planck divided phase space into elementary regions of action~\cite{planck1906}. Heisenberg's uncertainty principle $\sigma_x\,\sigma_p \ge \hbar/2$~\cite{kennard1927,robertson1929,schrodinger1930} fixes their minimum area but not their form, which a rotation of the axes redraws. Geometrically, de Gosson replaced the coordinate-dependent cell with the state's covariance ellipse of semi-axes $\Delta x = \sqrt{2}\,\sigma_x$ and $\Delta p = \sqrt{2}\,\sigma_p$. Its symplectic capacity $\pi\,\Delta x\,\Delta p$ is a canonical invariant, the area unchanged by rotation, squeeze, or shear~\cite{degosson2003quantization,degosson2009camel}. This is the action capacity (Figs.~\ref{fig:principal_blobs},~\ref{fig:quantum_blobs}),
\begin{equation}
...
Between these principal members the family $\{\beta_\theta\}$ deforms across $\theta \in [0,\pi)$ at constant action $h/2$ (Fig.~\ref{fig:quantum_blobs}). $A$ circumscribes each $\beta_\theta$. In the scaled coordinates $X = x/\Delta x$, $P = p/\Delta p$ every member is the same rigid ellipse rotated through $\theta$,
\begin{equation}
...
column 1) and the symplectic polar dual of the capacity $A$~\cite{degosson2022polar,degosson2024reconstruction}. The duality is reflexive, so $A$ and $a$ each fix the other, and antimonotone, so $a$ shrinks as $A$ grows. Each blob $\beta_\theta$ is circumscribed by $A$ and circumscribes $a$, nesting as $A \supseteq \beta_\theta \supseteq a$ (Fig.~\ref{fig:quantum_blobs}). Resolution and capacity are reciprocal at the quantum of action, $a\,A = (h/2)^2$.
