...
\caption{\textbf{Heisenberg cell, quadrature family of quantum blobs, and resolution cell.} The Heisenberg cell $A$ circumscribes the quantum blob family $\{\beta_\theta\}$, which circumscribes the resolution cell $a$. Across $\theta$ the family deforms at constant action $\beta_\theta = h/2$. At the principal angles $\theta = 0$ and $\theta = \pi/2$, the family recovers $\beta_{\theta=0}$ and $\beta_{\pi/2}$ from Fig.~\ref{fig:principal_blobs}.}
...
% ===============================================================================
Planck divided phase space into elementary regions of action~\cite{planck1906}. Heisenberg's uncertainty principle $\sigma_x\,\sigma_p \ge \hbar/2$~\cite{kennard1927,robertson1929,schrodinger1930} fixes their minimum area but not their form, which a rotation of the axes redraws. Geometrically, de Gosson recast the region as the state's covariance ellipse, whose symplectic capacity $\pi\,\Delta x\,\Delta p$ is a canonical invariant~\cite{degosson2003quantization,degosson2009camel}, with semi-axes $\Delta x = \sqrt{2}\,\sigma_x$ and $\Delta p = \sqrt{2}\,\sigma_p$. This is the Heisenberg cell (Figs.~\ref{fig:principal_blobs},~\ref{fig:quantum_blobs}) of action capacity,
\begin{equation}
...
Between these principal members the family $\{\beta_\theta\}$ deforms across $\theta \in [0,\pi)$ at constant action $h/2$ (Fig.~\ref{fig:quantum_blobs}). The Heisenberg cell $A$ circumscribes each $\beta_\theta$. In the scaled coordinates $X = x/\Delta x$, $P = p/\Delta p$ every member is the same rigid ellipse rotated through $\theta$,
\begin{equation}
...
\label{eq:Wtheta}
\end{equation}
lifts $W$ to Planck's quantum of action and renders $\widetilde W_\theta$ everywhere non-negative~\cite{cartwright1976,degosson2005bullsci,degossonluef2009irruption,cordero2018}.

%===============================================================================
%  ALL-ANGLES RESOLUTION CELL
%===============================================================================
Zurek also located the area scale $a \sim \hbar \times (\hbar / A)$~\cite{zurek2001,zurek2003decoherence} of the chessboard structure in the compass state. We identify its coordinate-free form as the resolution cell $a$ (Fig.~\ref{fig:cat}, column 1). It is the ellipse with semi-axes $\delta x, \delta p$ (Figs.~\ref{fig:principal_blobs},~\ref{fig:quantum_blobs}) and action
\begin{equation}
...
% ===============================================================================
A single blob commits to a quadrature the state does not favor. Integrating $\widetilde W_\theta$ over the family favors no quadrature, yielding the phase-space portrait,
\begin{equation}
...
resolution cell $a$, so no $h/2$ smoothing of this state resolves
